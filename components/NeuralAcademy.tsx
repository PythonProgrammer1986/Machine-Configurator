...
const NeuralAcademy: React.FC<Props> = ({ knowledgeBase, onKnowledgeBaseUpdate, apiKey, parts, rules, onRulesUpdate, glossary }) => {
  ...
  }, [parts, targetBom, useExistingBOM]);
  ...
  const startTraining = async () => {
    ...
    setIsTraining(true);
    setTrainingLog([]);
    addLog(`Academy Initiated: [${machineModel}]`, 'info');
    ...
    try {
      ...
      for (const file of moFiles) {
        ...
        const resp = await ai.models.generateContent({
          ...
          contents: {
            parts: [
              { text: `EXTRACT CONFIG OPTIONS FROM PAGES: ${pageRange}. JSON: {"options": [{"category": "string", "selection": "string"}]}` },
      ...
      addLog(`Extraction Complete. ${matches.length} Patterns found.`, 'success');
  ...
  const commit = () => {
    ...
    pendingMatches.forEach(pm => {
      ...
      if (pn) {
        ...
        if (ex !== -1) entries[ex].partNumber = pn;
        else entries.push({ category: pm.category, selection: pm.selection, partNumber: pn, confirmedCount: 1, lastUsed: new Date().toISOString() });
      }
    });
    onKnowledgeBaseUpdate(nKB);
    setPendingMatches([]);
    addLog(`Neural weights synced for [${m}]`, 'success');
  };
    ...
    <div className="flex flex-col h-full bg-slate-50">
      <div className="p-8 border-b bg-white flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-6">
          <div className="p-4 bg-indigo-600 text-white rounded-2xl shadow-xl"><GraduationCap size={32} /></div>
          <div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-[2.5rem] border p-8 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              <input type="text" placeholder="Profile Name" value={machineModel} onChange={e => setMachineModel(e.target.value.toUpperCase())} className="w-full bg-slate-50 border-2 rounded-2xl p-4 text-sm font-bold" />
              <div className="bg-slate-50 border-2 border-dashed rounded-2xl p-6 flex flex-col items-center justify-center relative cursor-pointer">
                ...
                <span className="text-[9px] font-black text-slate-400 mt-2 uppercase">{moFiles.length > 0 ? `${moFiles.length} Selected` : 'Upload PDFs/Images'}</span>
              </div>
              <button onClick={startTraining} disabled={isTraining} className={`w-full py-5 rounded-2xl flex items-center justify-center gap-3 text-xs font-black uppercase transition-all shadow-xl ${isTraining ? 'bg-slate-100 text-slate-400' : 'bg-indigo-600 text-white'}`}>
           ...
           {pendingMatches.length > 0 ? (
             <div className="bg-white rounded-[2.5rem] border-2 border-indigo-100 p-8 shadow-2xl">
                <div className="flex justify-between items-center mb-8">
                   <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Review Proposals</h3>
                   <div className="flex gap-4">
                      <button onClick={() => setPendingMatches([])} className="px-6 py-3 text-[10px] font-black uppercase bg-slate-50 rounded-xl">Discard</button>
                      <button onClick={commit} className="px-8 py-3 bg-indigo-600 text-white text-[10px] font-black uppercase rounded-xl">Commit Nodes</button>
                   </div>
                </div>
                         ...
                         {pendingMatches.map(m => (
                            <tr key={m.id}>
                               ...
                               <td className="px-6 py-4"><input type="text" value={m.manualPartNumber} onChange={e => {
                                  ...
                                  setPendingMatches(n);
                               }} className="bg-slate-50 border-2 rounded-xl px-4 py-2 text-xs font-mono font-bold w-full" /></td>
                            </tr>
                         ))}
                      </tbody>
                   </table>
             ...
             <div className="bg-white rounded-[2.5rem] border p-8 shadow-sm h-full min-h-[500px]">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-8"><BrainCircuit size={14} /> Knowledge Repository</h3>
                {stats.models === 0 ? <div className="h-full flex flex-col items-center justify-center text-slate-200 py-40"><History size={80} className="mb-6 opacity-20" /><p className="text-xs font-black uppercase tracking-[0.4em]">Reservoir Empty</p></div> : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {Object.keys(knowledgeBase).map(model => (
                      <div key={model} className="bg-slate-50 rounded-[2rem] p-8 border border-slate-100 group transition-all">
                        <div className="flex justify-between items-start mb-6">
                            <div className="p-3 bg-white rounded-2xl shadow-sm text-indigo-600"><Database size={20} /></div>
                            <button onClick={() => {
                              const n = { ...knowledgeBase }; delete n[model]; onKnowledgeBaseUpdate(n);
                            }} className="p-3 bg-white rounded-xl text-slate-400 hover:text-red-500 transition-all"><Trash2 size={16} /></button>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-8">
                           ...
                           <div className="px-4 py-2 bg-emerald-50 text-emerald-700 border rounded-xl text-[9px] font-black uppercase flex items-center gap-1.5"><ShieldCheck size={14} /> Ready</div>
                        </div>
                      </div>
                    ))}
                  </div>
